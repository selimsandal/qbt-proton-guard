...
func startBackground(binary, log string, arguments ...string) error {
	...
	for _, argument := range arguments {
		args += "<string>" + html.EscapeString(argument) + "</string>"
	}
	plist := fmt.Sprintf(`<?xml version="1.0" encoding="UTF-8"?><plist version="1.0"><dict><key>Label</key><string>%s</string><key>ProgramArguments</key><array><string>%s</string>%s</array><key>RunAtLoad</key><true/><key>StandardOutPath</key><string>%s</string><key>StandardErrorPath</key><string>%s</string></dict></plist>`, label, html.EscapeString(binary), args, html.EscapeString(log), html.EscapeString(log))
